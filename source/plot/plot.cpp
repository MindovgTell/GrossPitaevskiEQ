...
namespace gpes::plot {
    ...
    inline void draw(Grid<Dimension::One>& grid, std::string xlabel, std::string ylabel){
        try {
            int size = grid.size();
            if (size <= 0) {
    ...
    // inline void draw(std::shared_ptr<const Grid<Dimension::One>> grid, std::string xlabel, std::string ylabel){
    //     if (!grid) {
